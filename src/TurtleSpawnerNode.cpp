...
TurtleSpawnerNode::TurtleSpawnerNode() : Node("turtle_spawner"), turtle_counter_(0)
    {
        this->declare_parameter("turtle_name_prefix", "NinjaTurtle");
...
double TurtleSpawnerNode::randomDouble()
    {
        return double(std::rand()) / (double(RAND_MAX) + 1.0);
    }

void TurtleSpawnerNode::spawnNewTurtle()
    {
        turtle_counter_++;
        ...
        spawn_turtle_threads_.push_back(
            std::make_shared<std::thread>(
                std::bind(&TurtleSpawnerNode::callSpawnTurtleService, this,name, x, y, theta)));
    }

void TurtleSpawnerNode::callSpawnTurtleService(const std::string& name, double x, double y, double theta)
    {
...
void TurtleSpawnerNode::callbackCatchTurtle(const my_robot_interfaces::srv::CatchTurtle::Request::SharedPtr request,
...
void TurtleSpawnerNode::callKillTurtleService(std::string turtle_name)
    {
